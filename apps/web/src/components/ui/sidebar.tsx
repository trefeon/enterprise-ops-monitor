  ...
  return (
    <SidebarContext.Provider value={{ open, setOpen, toggleSidebar }}>
      <div
        ...
        className={cn(
          "group/sidebar-wrapper flex min-h-svh w-full bg-background text-foreground [--sidebar-width:15rem] [--sidebar-width-icon:4.75rem]",
          className
        )}
        style={style}
        {...props}
  ...
  return (
    <aside
      ...
      className={cn(
        ...
        collapsible === "icon" ? (open ? "w-[--sidebar-width]" : "w-[--sidebar-width-icon]") : "w-[--sidebar-width]",
...
function SidebarMenuBadge({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn("ml-auto rounded bg-sidebar-accent px-1.5 py-0.5 text-[10px] text-sidebar-accent-foreground", className)}
      {...props}
    />
  );
}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn("ml-5 grid gap-1 border-l border-sidebar-border pl-2 group-data-[state=collapsed]/sidebar-wrapper:hidden", className)}
      {...props}
    />
  );
}
...
function SidebarMenuSubButton({
  ...
  return (
    <button data-slot="sidebar-menu-sub-button" data-active={isActive} className={classes} {...props}>
      {children}
    </button>
  );
}
