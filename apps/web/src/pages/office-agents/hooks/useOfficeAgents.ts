...
export function useOfficeAgents() {
  ...
  const filtered = machines.filter((m) => {
    ...
    if (search) {
      ...
      if (!hostname.includes(q) && !label.includes(q) && !cpu.includes(q) && !os.includes(q)) return false
    }
    return true
  })
  ...
  const stats = {
    ...
    healthy: machines.filter((m) => Math.max(m.metrics.cpu_percent, m.metrics.ram_percent, m.metrics.disk_percent) < 70).length,
    warning: machines.filter((m) => {
      const max = Math.max(m.metrics.cpu_percent, m.metrics.ram_percent, m.metrics.disk_percent)
      return max >= 70 && max < 90
    }).length,
    critical: machines.filter((m) => Math.max(m.metrics.cpu_percent, m.metrics.ram_percent, m.metrics.disk_percent) >= 90).length,
  }
