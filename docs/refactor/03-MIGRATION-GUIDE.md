...
- [x] Migrate `PageHeader` import to `@/components/shared/PageHeader`
- [x] Migrate `StatCard` import to `@/components/shared/StatCard`
- [x] Migrate `StatusBadge` import to `@/components/shared/StatusBadge`
- [x] Migrate `DataTable` import to `@/components/shared/DataTable`
- [x] Migrate `EmptyState` import to `@/components/shared/EmptyState`
- [x] Migrate `PageShell` import to `@/components/shared/PageShell`

### 3.2 EOD Monitor (`EODMonitor/index.jsx`)

- [x] Migrate `PageHeader`, `Toolbar`, `IconButton`, `ProgressBar`, `PageShell`, `Modal`, `StatCard`, `StatusBadge`, `SearchBar`, `DatePicker`, `ConfirmDialog` imports
- [x] Replace custom inline main table structure with `<DataTable>`
- [x] Replace custom inline branch modal table structure with `<DataTable>`

### 3.3 Store Sync (`StoreSync/index.jsx`)

- [x] Migrate `PageHeader`, `StatCard`, `StatusBadge`, `ProgressBar`, `EmptyState`, `Modal`, `PageShell`, `SearchBar` imports
- [x] Replace custom inline store sync status list/table with `<DataTable>`

### 3.4 System Health (`SystemHealth/index.jsx`)

- [x] Migrate `PageHeader`, `ProgressBar`, `StatusBadge`, `EmptyState`, `PageShell`, `ConfirmDialog` imports
- [x] Replace custom inline system/health check logs table with `<DataTable>`

### 3.5 Backups (`Backups/index.jsx`)

- [x] Migrate `StatusBadge`, `ProgressBar`, `EmptyState`, `IconButton`, `PageShell`, `ConfirmDialog` imports
- [x] Replace custom inline backup list table with `<DataTable>`

### 3.6 Office Agents (`office-agents/index.tsx`)

- [x] Migrate `PageHeader`, `StatCard`, `SearchBar`, `PageShell` imports

### 3.7 Agent Updater (`AgentUpdater/index.jsx`)

- [x] Migrate `PageHeader`, `StatCard`, `StatusBadge`, `EmptyState`, `Modal`, `PageShell` imports

### 3.8 Store Management (`StoreManagement/index.jsx`)

- [x] Migrate `PageHeader`, `DataTable`, `Toolbar`, `EmptyState`, `PageShell` imports

### 3.9 Users Admin (`UsersAdmin/index.jsx`)

- [x] Migrate `PageHeader`, `DataTable`, `Modal`, `ConfirmDialog`, `PageShell` imports

### 3.10 Identity Check (`IdentityCheck/index.jsx`)

- [x] Migrate `DataTable`, `Toolbar`, `EmptyState`, `PageShell` imports

### 3.11 After Hours & After Hours Report (`AfterHours/index.jsx` & `AfterHoursReport/index.jsx`)

- [x] Migrate `PageHeader`, `Toolbar`, `StatCard`, `StatusBadge`, `EmptyState`, `PageShell`, `ConfirmDialog` imports

### 3.12 Columns & Helper Files

- [x] `columns/eodMonitorColumns.jsx`: Migrate `StatusBadge`, `IconButton`
- [x] `columns/storeColumns.jsx`: Migrate `StatusBadge`, `IconButton`
- [x] `columns/backupColumns.jsx`: Migrate `StatusBadge`, `IconButton`
- [x] `PageLoader.jsx`: Migrate `PageShell`
