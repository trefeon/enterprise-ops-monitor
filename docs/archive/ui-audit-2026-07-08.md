...
> Dated 2026-07-08, this audit describes the Geist `--ds-*` token system that was replaced by the
> Supabase design-system migration (commit `761357e`). Verified 2026-08-11: the `--ds-*` tokens and the
> `rounded-xl`/`rounded-2xl` findings no longer exist in code, and `text-3xs` — flagged here as a
> violation — is a defined token (`apps/web/tailwind.config.js`). Superseded by roadmap task R2.1.
